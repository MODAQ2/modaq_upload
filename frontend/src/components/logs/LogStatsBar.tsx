  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
      <StatCard
        ...
        color="text-nlr-blue"
      />
      <StatCard value={stats.today_entries.toLocaleString()} label="Today" color="text-nlr-green" />
      <StatCard
        value={(stats.level_counts.ERROR ?? 0).toLocaleString()}
        label="Errors"
        color="text-red-500"
      />
      <StatCard value={stats.file_count.toLocaleString()} label="Log Files" color="text-gray-600" />
    </div>
  );
}
